...
Partial Class QLT_BAOCAO_THUOC_TUYENTINH
    ...
    Private Sub BindGrid()
        ...
        Try
            ...
            For Each itm In Me.grdDS.Items
                If CInt(itm.Cells(4).Text) = 1 Then
                    itm.Cells(8).Enabled = False
                End If
            Next
    ...
    Protected Sub grdDS_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles grdDS.ItemCommand
        ...
        If e.CommandName = "ketchuyen" Then
            Dim obj As New NTP_QLT.QLT_KY_KIEMKE_BO
            Try
                obj.KetChuyenSoLieu(CInt(e.Item.Cells(2).Text), CInt(e.Item.Cells(3).Text), Me.CurrentUserStock.ID_KHO_THUOC)
                DotNetNuke.UI.Skins.Skin.AddModuleMessage(Me, "Kết chuyển số liệu thành công", Skins.Controls.ModuleMessage.ModuleMessageType.GreenSuccess)
                BindGrid()
            Catch ex As Exception
                DotNetNuke.UI.Skins.Skin.AddModuleMessage(Me, "Không kết chuyển được số liệu", Skins.Controls.ModuleMessage.ModuleMessageType.RedError)
    ...
    Private Sub BaoCao(ByVal ID_KHO As Integer, ByVal ID_KY_KIEMKE As Integer)
        'Xem bao cao kiem ke
        Dim url As String
        url = Request.ApplicationPath & "/DesktopModules/NTP_QLT/QLT_BAOCAO_TONGHOP_KIEMKE.aspx?ID_KHO=" & ID_KHO & "&ID_KY_KIEMKE=" & ID_KY_KIEMKE ' "./DesktopModules/VTP_VIEWREPORT/ShowReport.aspx"
