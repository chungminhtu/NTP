<%@ Control Language="VB" AutoEventWireup="false" CodeFile="NTP_QLT_BC_SUDUNGTHUOC_EDIT.ascx.vb" Inherits="NTP_QLT_BC_SUDUNGTHUOC_EDIT" %>
...
function initform()
{
    ...
    // hoa chat
   // $('input[id^=<%=TD_LOSX.ClientID %>]').blur(function;
   // $('input[id^=<%=TD_HANDUNG.ClientID %>]').bind('keyup',recalc);
   // $('input[id^=<%=X_LOSX.ClientID %>]').bind('keyup',recalc);
   // $('input[id^=<%=X_HANDUNG.ClientID %>]').bind('keyup',recalc);
   // $('input[id^=<%=TC_LOSX.ClientID %>]').bind('keyup',recalc);
   // $('input[id^=<%=TC_HANDUNG.ClientID %>]').bind('keyup',recalc);
    
   // $("input[id=<%= Me.TD_LOSX.ClientID %>]").blur(function (){
    //     $('input[id=<%=X_LOSX.ClientID %>]').val($('input[id^=<%=TD_LOSX.ClientID %>]').val());
    //     $('input[id=<%=N_LOSX.ClientID %>]').val($('input[id^=<%=TD_LOSX.ClientID %>]').val());
     //}
     //);
     
     $("input[id=<%= Me.TD_LOSX.ClientID %>]").blur(function (){
        setvalue($("input[id=<%= Me.TD_LOSX.ClientID %>]"), $('input[id=<%=X_LOSX.ClientID %>]'),$('input[id=<%=N_LOSX.ClientID %>]'));
     }
     );
     
       $("input[id=<%= Me.N_LOSX.ClientID %>]").blur(function (){
        setvalue($("input[id=<%= Me.N_LOSX.ClientID %>]"), $('input[id=<%=TD_LOSX.ClientID %>]'),$('input[id=<%=X_LOSX.ClientID %>]'));
     }
     );
     
       $("input[id=<%= Me.X_LOSX.ClientID %>]").blur(function (){
        setvalue($("input[id=<%= Me.X_LOSX.ClientID %>]"), $('input[id=<%=TD_LOSX.ClientID %>]'),$('input[id=<%=N_LOSX.ClientID %>]'));
     }
     );
     
     
       $("input[id=<%= Me.TD_HANDUNG.ClientID %>]").blur(function (){
        setvalue($("input[id=<%= Me.TD_HANDUNG.ClientID %>]"), $('input[id=<%=N_HANDUNG.ClientID %>]'),$('input[id=<%=X_HANDUNG.ClientID %>]'));
     }
     );
     
     $("input[id=<%= Me.N_HANDUNG.ClientID %>]").blur(function (){
        setvalue($("input[id=<%= Me.N_HANDUNG.ClientID %>]"), $('input[id=<%=TD_HANDUNG.ClientID %>]'),$('input[id=<%=X_HANDUNG.ClientID %>]'));
     }
     );
     
          $("input[id=<%= Me.X_HANDUNG.ClientID %>]").blur(function (){
        setvalue($("input[id=<%= Me.X_HANDUNG.ClientID %>]"), $('input[id=<%=N_HANDUNG.ClientID %>]'),$('input[id=<%=TD_HANDUNG.ClientID %>]'));
     }
     );
});
}

function setvalue(objsource,des1,des2)
{
    des1.val(objsource.val());
    des2.val(objsource.val());
}
...
<table  class = "ntp_table_main" width = 100% >
    <tr>
        <td colspan="6" width="15%">
            <asp:Label ID="Label5" runat="server" CssClass="ntp_comment" Text="Chú ý: <br>Số lượng tồn cuối sẽ tự tính theo công thức : tồn cuối = (tồn đầu + số lượng nhập + trả lại + thừa) - (xuất sử dụng + hỏng vỡ/thiếu + điều chuyển) <br> Lô sản xuất/Hạn dùng của lượng tồn đầu/nhập/xuất phải giống nhau"></asp:Label></td>
    </tr>
     <tr>
        <td width=25%>
            <asp:Label ID="Label1" runat="server" CssClass="ntp_label" Text="Chọn đơn vị"></asp:Label></td>
         <td width="15%" colspan="5">
         <asp:DropDownList ID="cboHuyen" runat="server" CssClass="ntp_combobox" DataTextField="TEN_BENHVIEN"
                  DataValueField="ID_BENHVIEN" TabIndex="2" Width="100%" AutoPostBack="True">
        </asp:DropDownList></td>
    </tr>
    <tr>
        <td width="25%">
    ...
    <tr>
        ...
        <td width="15%">
            <asp:Label ID="Label15" runat="server" CssClass="ntp_label" Text="Lô sản xuất"></asp:Label></td>
        <td width="15%">
    ...
    <tr>
        ...
        <td width="15%">
            <asp:TextBox ID="TD_LOSX" runat="server" CssClass="ntp_textbox" MaxLength="50" TabIndex="6"
                Width="100px"></asp:TextBox>
            <asp:RequiredFieldValidator ID="RequiredFieldValidator2" runat="server" ControlToValidate="TD_LOSX"
                ErrorMessage="Nhập lô SX" ValidationGroup="DETAIL"></asp:RequiredFieldValidator></td>
        <td width="15%">
            <asp:TextBox ID="TD_HANDUNG" runat="server" CssClass="ntp_textbox" MaxLength="50"
                TabIndex="7" Width="100px"></asp:TextBox><ajaxToolkit:MaskedEditExtender ID="Maskededitextender2"
            ...
            <ajaxToolkit:MaskedEditValidator ID="Maskededitvalidator2" runat="server" ControlExtender="Maskededitextender2"
                ...
                SetFocusOnError="True" TooltipMessage="Nhập ngày" ValidationGroup="DETAIL"></ajaxToolkit:MaskedEditValidator>
            <asp:RequiredFieldValidator ID="RequiredFieldValidator3" runat="server" ControlToValidate="TD_HANDUNG"
                ErrorMessage="Nhập hạn dùng" ValidationGroup="DETAIL"></asp:RequiredFieldValidator></td>
                <td  width="15%"></td>
                <td  width="15%"></td>
    ...
    <tr>
        ...
        <td width="15%">
            <asp:Label ID="lblTraLai" runat="server" CssClass="ntp_label" Text="Trả lại"></asp:Label></td>
        <td width="15%">
            <asp:Label ID="lblThua" runat="server" CssClass="ntp_label" Text="Thừa"></asp:Label></td>
    </tr>
    <tr>
        <td width="15%">
            ...
            <asp:RangeValidator ID="RangeValidator3" runat="server" ControlToValidate="N_SOLUONG"
                ...
                Type="Integer" ValidationGroup="DETAIL"></asp:RangeValidator></td>
        <td width="15%">
            <asp:TextBox ID="N_LOSX" runat="server" CssClass="ntp_textbox" MaxLength="50" TabIndex="9"
                Width="100px"></asp:TextBox>
            <asp:RequiredFieldValidator ID="RequiredFieldValidator4" runat="server" ControlToValidate="N_LOSX"
                ErrorMessage="Nhập lô SX" ValidationGroup="DETAIL"></asp:RequiredFieldValidator></td>
        <td width="15%">
            <asp:TextBox ID="N_HANDUNG" runat="server" CssClass="ntp_textbox" MaxLength="50"
                TabIndex="10" Width="100px"></asp:TextBox><ajaxToolkit:MaskedEditExtender ID="Maskededitextender1"
                    runat="server" AutoComplete="true" CultureName="vi-VN" Mask="99/99/9999" MaskType="Date"
            ...
            <ajaxToolkit:MaskedEditValidator ID="MaskedEditValidator1" runat="server" ControlExtender="Maskededitextender1"
                ...
                MinimumValue=" 1/1/1753" MinimumValueMessage="Bạn chỉ được nhập ngày trong khoảng (1/1/1753 - 31/12/9999)"
                SetFocusOnError="True" TooltipMessage="Nhập ngày" ValidationGroup="DETAIL"></ajaxToolkit:MaskedEditValidator>
            <asp:RequiredFieldValidator ID="RequiredFieldValidator6" runat="server" ControlToValidate="N_HANDUNG"
                ErrorMessage="Nhập hạn dùng" ValidationGroup="DETAIL"></asp:RequiredFieldValidator></td>
        <td width="15%">
            <asp:TextBox ID="N_TRALAI" runat="server" CssClass="ntp_textbox" MaxLength="50" TabIndex="11"
                Width="100px"></asp:TextBox>
            <asp:RangeValidator ID="RangeValidator4" runat="server" ControlToValidate="N_TRALAI"
                Display="Dynamic" ErrorMessage="Nhập số" MaximumValue="99999999" MinimumValue="0"
                Type="Integer" ValidationGroup="DETAIL"></asp:RangeValidator></td>
        <td width="15%">
            <asp:TextBox ID="N_THUA" runat="server" CssClass="ntp_textbox" MaxLength="50" TabIndex="12"
                Width="100px"></asp:TextBox>
    ...
    <tr>
        ...
        <td width="15%" style="height: 21px">
            <asp:Label ID="Label10" runat="server" CssClass="ntp_label" Text="Lô sản xuất"></asp:Label></td>
        <td width="15%" style="height: 21px">
            <asp:Label ID="Label14" runat="server" CssClass="ntp_label" Text="Hạn dùng"></asp:Label></td>
        <td width="15%" style="height: 21px">
            &nbsp;<asp:Label ID="lblX_HONGVO" runat="server" CssClass="ntp_label" Text="Hỏng vỡ thiếu"></asp:Label></td>
        <td width="15%" style="height: 21px">
            &nbsp;<asp:Label ID="lblX_DIEUCHUYEN" runat="server" CssClass="ntp_label" Text="Điều chuyển"></asp:Label>&nbsp;
        </td>
    </tr>
    <tr>
        <td width="15%">
            <asp:TextBox ID="X_SUDUNG" runat="server" CssClass="ntp_textbox" MaxLength="50" TabIndex="13"
                Width="100px"></asp:TextBox>
            <asp:RangeValidator ID="RangeValidator6" runat="server" ControlToValidate="X_SUDUNG"
                Display="Dynamic" ErrorMessage="Nhập số" MaximumValue="99999999" MinimumValue="0"
                Type="Integer" ValidationGroup="DETAIL"></asp:RangeValidator></td>
        <td width="15%">
            <asp:TextBox ID="X_LOSX" runat="server" CssClass="ntp_textbox" MaxLength="50" TabIndex="14"
                Width="100px"></asp:TextBox>
            <asp:RequiredFieldValidator ID="RequiredFieldValidator5" runat="server" ControlToValidate="X_LOSX"
                ErrorMessage="Nhập lô SX" ValidationGroup="DETAIL"></asp:RequiredFieldValidator></td>
        <td width="15%">
            <asp:TextBox ID="X_HANDUNG" runat="server" CssClass="ntp_textbox" MaxLength="50" TabIndex="15"
                Width="100px"></asp:TextBox>
            <ajaxToolkit:MaskedEditExtender ID="MaskedEditExtender3"
                    runat="server" AutoComplete="true" CultureName="vi-VN" Mask="99/99/9999" MaskType="Date"
                    PromptCharacter="_" TargetControlID="X_HANDUNG">
            </ajaxToolkit:MaskedEditExtender>
            <ajaxToolkit:MaskedEditValidator ID="MaskedEditValidator3" runat="server" ControlExtender="MaskedEditExtender3"
                ControlToValidate="X_HANDUNG" EmptyValueMessage="Bạn phải nhập ngày" InvalidValueMessage="Định dạng ngày không đúng"
                MaximumValue="31/12/9999" MaximumValueMessage="Bạn chỉ được nhập ngày trong khoảng (1/1/1753 - 31/12/9999)"
                MinimumValue=" 1/1/1753" MinimumValueMessage="Bạn chỉ được nhập ngày trong khoảng (1/1/1753 - 31/12/9999)"
                SetFocusOnError="True" TooltipMessage="Nhập ngày" ValidationGroup="DETAIL"></ajaxToolkit:MaskedEditValidator>
            <asp:RequiredFieldValidator ID="RequiredFieldValidator7" runat="server" ControlToValidate="X_HANDUNG"
                ErrorMessage="Nhập hạn dùng" ValidationGroup="DETAIL"></asp:RequiredFieldValidator></td>
        <td width="15%">
            <asp:TextBox ID="X_HONG" runat="server" CssClass="ntp_textbox" MaxLength="50" TabIndex="16"
                Width="100px"></asp:TextBox>
    ...
    <tr>
        ...
        <td colspan="5">
            <asp:TextBox ID="txtGhiChu" runat="server" CssClass="ntp_textbox" MaxLength="50"
                TabIndex="18" TextMode="MultiLine" Width="100%"></asp:TextBox></td>
    </tr>
    <tr>
        <td rowspan="1" width="25%">
        </td>
        <td colspan="5">
            <asp:Button ID="cmdSaveDetail" runat="server" Text="Ghi chi tiết" CssClass="ntp_button" Width="100px" TabIndex="19" ValidationGroup="DETAIL" />&nbsp;
