...
{	
	...
	function GetDataFromServer(sValue)
	{
		var sUrl;
		//alert("ok");
		 //alert(this.msDataPage);
		sUrl=this.msResourcesDir + 
				"/" + this.msDataPage + "?TextBoxID=" + encodeURIComponent(this.msTextBoxID) + 
	...
	function SetTextBoxValue()
	{
		...
		if(divMenuItem)
		{
			...
			//<DatNL - 08/01/2008 - Chi lay value sau dau ",">
			//=> ko set ca text box ma add them sau dau "," cuoi cung
			var sValue = txtCtrl.value;
			var arr = sValue.split(sSpliter);
			//thay phan tu cuoi cung
			if (arr.length > 0) {
			    arr[arr.length - 1] = GetInnerHtml(divMenuItem);
			};
			
			//Noi het cac phan tu arr lai. Phan cach = dau phay
			txtCtrl.value = arr.join(sSpliter);
	...
	function GetTextBoxValue()
	{
		...
		//<DatNL - 08/01/2008 - Chi lay value sau dau ",">
		var sReturn = txtCtrl.value;
		
		var arr = sReturn.split(",");
		
		if (arr.length>0){
		    TRACE(arr.length);
		    sReturn = arr[arr.length-1];
		};
	...
	function OnKeyUp(evt,DataPage)
	{
		...
		if ((nKey!=38) && (nKey!=40) && (nKey!=13))
		{
			...
			if ((sNewValue.length <= this.mnMaxSuggestChars) && (sNewValue.length >= this.mnMinSuggestChars))
			{
	...
	function OnBlur()
	{
	    ...
	    window.setTimeout("if (this.HideMenuDiv){this.HideMenuDiv();this.mbHasFocus=false;}", 150);
		//this.HideMenuDiv();
	...
	{
		if (this.GetMenuDiv().style.visibility == 'hidden') return false;
		else return true;		
	}
